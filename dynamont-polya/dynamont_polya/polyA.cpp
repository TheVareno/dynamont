 ...
 */
vector<size_t> getBorders(const double* LPS, const double* LPL, const double* LPA, const double* LPPA, const double* LPTR, const size_t T){
...
template <typename T>
void writeBorders(const string& save_file, const string& read_id, const vector<T>& borders) {
    ...
    output_file << read_id << ","; 
    output_file << borders[0] << ",";
    output_file << borders[1];
    output_file << endl;
    output_file.close();
}
...
int main() {
    ...
    writeBorders(save_file, read_id, borders);
