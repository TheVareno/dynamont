...
def main():
    ...
    parser.add_argument('--input_dir', type=str, help='path to directory of all coordinations datasets')
    ...
    parser.add_argument('--save_path', type=str, help='the path to save')
    ...
    if args.input_dir: 
        ...
        for dataset in datasets: 
            if os.path.isfile(dataset) and dataset.endswith('.csv') :
                if dataset.startwith('ann'):
                    ann_coord = dataset
                if dataset.startwith('tail'):
                    tailfindr_coord = dataset
                else:
                    nnps_coord = dataset

        # csv --> pd.DataFrame
        ann_coord = load_coordination_dataset(ann_coord)
        ...
        tailfindr_coord = load_coordination_dataset(tailfindr_coord)
        ...
        nnps_coord = load_coordination_dataset(nnps_coord)
