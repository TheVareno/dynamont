...
def setup_working_directory():     
    if os.getcwd() != '//home/hi68ren/Dokumente/dynamont-fork/dynamont/dynamont-polya/main' : 
        os.chdir('/home/hi68ren/Dokumente/dynamont-fork/dynamont/dynamont-polya/main')
    else : 
        pass
    ...
    while not task_queue.empty(): 
        try:
            ...
            if len(filtered_signal_values) == 0:
                print(f"[ERROR] the array of signal values empty for read id : {read_id}")
            ...
            if not sig_vals_str: 
                print(f"[ERROR] empty signal values for read {read_id}")
...
def main(): 
    ...
    parser.add_argument("--input_dir", type=str, help="Path to ONT read data in either FAST5, POD5 or SLOW5.")
    parser.add_argument("--output_dir", type=str, help="Path to save output file.")
    args = parser.parse_args()

    file_path = args.input_dir
    
    save_file = os.path.join(args.output_dir, 'region_borders.txt') 
    
    if not os.path.isfile(file_path): 
        raise FileNotFoundError(f'The input file {args.input_dir} not found.')
    ...
    processes = [ mp.Process(target=find_polya, args=(task_queue, result_queue, file_path)) 
